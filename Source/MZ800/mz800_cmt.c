...
bool mz800_sys_load_mzf(mz800_sys_t* sys, const uint8_t* data, uint32_t size) {
    if (!data || size < CMT_MZF_HEADER_SIZE) return false;

    // Load tape data into CMT chip (copies header + body into embedded arrays)
    if (!cmt_load_mzf(&sys->cmt, data, size)) return false;
